...
class QueryService:
    ...
    def __init__(self, neo4j_client, spatial_intel=None, memory=None, metrics_collector=None):
        ...
        # Inherit city config from spatial_intel
        self._city_cfg = Config.get_city_config(getattr(self.spatial_intel, 'city', 'Philadelphia'))
        # Initialize LLM
        self.llm = ChatGroq(
        ...
        self.service_synonyms = {
            ...
            'benefit': ['benefit', 'benefits', 'retirement', 'disability', 'ssi', 'medicare', 'social security',
                        'apply for benefits'],
            ...
            'change': ['change', 'update', 'modify', 'direct deposit', 'change address', 'change direct deposit',
            ...
            'computer': ['computer', 'computers', 'public computers', 'computer access', 'computer labs',
            ...
            'class': ['class', 'classes', 'education', 'learning', 'computer class', 'health education',
                      'sex education', 'parenting education'],
            'ged': ['ged', 'adult education', 'basic literacy', 'literacy'],
            'homework': ['homework', 'homework help', 'tutoring', 'study'],
            'job': ['job', 'job assistance', 'job search', 'job readiness', 'workforce development', 'employment',
                    'help find work', 'resume development'],
            ...
            'shelter': ['stay', 'shelter', 'housing', 'safe housing', 'short-term housing', 'residential housing',
                        'help find housing'],
            'food': ['food', 'meals', 'meal', 'emergency food', 'food pantry', 'nutrition', 'food delivery'],
            'mental health': ['mental health', 'mental health care', 'counseling', 'therapy', 'psychiatric',
                              'support groups', 'peer support', 'bereavement', 'anger management', 'group therapy'],
            'substance abuse': ['substance abuse', 'addiction', 'recovery', 'sober living', 'detox', '12-step',
                                'outpatient treatment'],
            'financial': ['financial', 'financial assistance', 'emergency payments', 'pay for housing',
                          'pay for utilities', 'government benefits'],
    ...
    def process_query_with_coordinates(self, user_query, coordinates):
        ...
        try:
            ...
            city_state = self._city_cfg["geocode_suffix"]
            city_name = city_state.split(",")[0].strip()
            city_state_code = self._city_cfg["state"].lower()
            location_text = "user location" if coordinates != default_coords else f"{city_name} city center (default)"
...
- This dataset covers {city_state} ONLY.
- You MUST include: toLower(l.state) = '{city_state_code}' in the WHERE clause.
...
"""
            ...
            if self.metrics and 'neo4j_duration' in query_result:
                self.metrics.record_query_result(
                    ...
                    # Now correctly reflects if expansion happened
    ...
    **Temporary Shelter Services:**
    ...
    def _validate_spatial_cypher(self, cypher_query):
        ...
        for pattern in problematic_patterns:
            if re.search(pattern, query_lower):
                ...
                logging.warning(
                    f"This may cause incorrect results. Spatial queries should only use distance filtering.")
    ...
    def _process_spatial_query(self, query):
        ...
        try:
            ...
            logging.info(
                f"Spatial processing successful: {location_text} -> {user_coordinates}, threshold: {distance_threshold}")
            logging.info(
                f"Spatial processing time: {total_spatial_duration:.3f}s (geocoding: {geocoding_duration:.3f}s)")
    ...
    def _clean_cypher_response(self, cypher_response_text):
        ...
        lines = cypher_text.split('\n')
        cypher_start_keywords = ['MATCH', 'OPTIONAL', 'WITH', 'CREATE', 'MERGE', 'DELETE', 'DETACH', 'SET', 'REMOVE',
                                 'RETURN', 'CALL', 'USING', 'UNWIND']
        ...
        logging.info(
            f"Cleaned Cypher query from LLM response. Original length: {len(cypher_response_text)}, Cleaned length: {len(cypher_text)}")
