...
class OrganizationInfoApp:
    ...
    def __init__(self, session_id=None, city: str = "Philadelphia"):
        ...
        self.city = city
        self._city_cfg = Config.get_city_config(city)
        ...
        # Initialize core components (city-aware)
        self.spatial_intel = SpatialIntelligence(city=city)
        ...
        logging.info(f"OrganizationInfoApp initialized for session: {self.session_id}, city: {self.city}")
    ...
    def process_user_request_for_streamlit(self, user_query, user_location=None):
        ...
        default_coords = self._city_cfg["default_coords"]
        ...
        if is_personal_location:
            ...
            if user_location:
                ...
                # Scenario 1 variant: No permission but personal reference - use city center as default
                logging.info(
                    f"Scenario 1 variant: No location permission + personal reference - using {self.city} default")
                should_call_spatial_intel = False
                final_coordinates = default_coords
        else:
            ...
            if user_location:
                ...
                if has_specific_location:
                    ...
                    logging.info(
                        "Scenario 2: No location permission + specific area mentioned - using spatial intelligence")
                    should_call_spatial_intel = True
                else:
                    # Scenario 1: No permission + no specific location - use city center as default
                    logging.info(f"Scenario 1: No location permission + no specific area - using {self.city} default")
                    should_call_spatial_intel = False
                    final_coordinates = default_coords
...
def display_structured_response(response_data, raw_data=None, user_query="", app_instance=None, message_index=None,
                                start_coordinates=None, all_categories_data=None, current_category_index=None):
    """
...
def get_session_app():
    ...
    selected_city = st.session_state.get('selected_city', 'Philadelphia')

    # Rebuild app if city changed
    if ('app_instance' not in st.session_state or
            st.session_state.get('_app_city') != selected_city):
        import uuid
        session_id = str(uuid.uuid4())
        st.session_state.session_id = session_id
        st.session_state._app_city = selected_city

        try:
            st.session_state.app_instance = OrganizationInfoApp(
                session_id=session_id, city=selected_city
            )
            logging.info(f"Created new app instance for session: {session_id[:8]}..., city: {selected_city}")
        except Exception as e:
...
def get_user_location_for_directions(override_start_coords=None, leg_key_suffix=""):
    ...
    # Priority 4: Default to city center coordinates
    active_city = st.session_state.get('selected_city', 'Philadelphia')
    default_coords = Config.get_city_config(active_city)["default_coords"]
    logging.info(f"Using default {active_city} city center for directions: {default_coords}")
...
def geocode_start_location(address):
    ...
    try:
        ...
        active_city = st.session_state.get('selected_city', 'Philadelphia')
        geocode_suffix = Config.get_city_config(active_city)["geocode_suffix"]
        search_query = f"{address}, {geocode_suffix}" if geocode_suffix.split(",")[0] not in address else address
...
def display_embedded_directions_for_all_organizations(raw_data, user_query="", app_instance=None, message_index=None,
                                                      start_coordinates=None, all_categories_data=None,
                                                      current_category_index=None, shared_selection_key=None):
    """
    ...
    try:
                ...
                for record in prev_leg_data:
                    ...
                    if org_name == selected_org_name:
                        ...
                        if lat and lon:
                            ...
                            logging.info(
                                f"Dynamic routing: Using coordinates from {selected_org_name}: {dynamic_start_coordinates}")
                            break
        ...
        for record in limited_raw_data:
            ...
            if org_name:
                ...
                city = (
                        ...
                        record.get('city') or
                        st.session_state.get('selected_city', 'Philadelphia')
                )

                state = (
                        record.get('l.state') or
                        record.get('state') or
                        Config.get_city_config(st.session_state.get('selected_city', 'Philadelphia'))["state"]
                )
        ...
        if selected_org and selected_org in org_data:
            ...
            placeholder_text = Config.get_city_config(st.session_state.get('selected_city', 'Philadelphia'))[
                "address_placeholder"]
            ...
            if custom_location_key in st.session_state and st.session_state[custom_location_key]:
                ...
                custom_lat, custom_lon = st.session_state[custom_location_key]
                default_location_text = st.session_state.get(custom_location_name_key,
                                                             f"Custom ({custom_lat:.4f}, {custom_lon:.4f})")
            ...
            if set_location and custom_address:
                with st.spinner("Processing location..."):
                    ...
                    if start_lat and start_lon:
                        ...
                        logging.info(
                            f"Custom start location set for leg {key_suffix}: {full_address or custom_address}")
                        st.rerun()
                    else:
            ...
            if temp_dropdown_key in st.session_state:
                ...
                temp_selected_org = temp_selected_display.split(". ", 1)[
                    1] if ". " in temp_selected_display else temp_selected_display
                ...
                if temp_selected_org != st.session_state.get(shared_selection_key):
                    logging.info(
                        f"Dropdown out of sync: '{temp_selected_org}' vs shared '{st.session_state.get(shared_selection_key)}'")
            ...
            selected_org_new = selected_display_new.split(". ", 1)[
                1] if ". " in selected_display_new else selected_display_new
...
@st.cache_data
def geocode_address(address, org_name=None, city: str = "Philadelphia"):
    ...
    try:
        ...
        city_cfg = Config.get_city_config(city)
        geocode_suffix = city_cfg["geocode_suffix"]
        city_name = geocode_suffix.split(",")[0].strip()
        ...
        if city_name in address:
            simplified_address = address.split(",")[0] + f", {geocode_suffix}"
        ...
        if city_name in address:
            very_simple = address.split(",")[0] + f", {city_name}"
            location = geolocator.geocode(very_simple)
            if location:
                return location.latitude, location.longitude

        # If address fails completely, try the organization name + city
        if org_name:
            org_location = geolocator.geocode(f"{org_name}, {geocode_suffix}")
            if org_location:
    ...
    with st.chat_message(message["role"]):
        ...
        if isinstance(message["content"], dict) and message["content"].get("type") == "multi_category_complete":
            for cat_idx, cat_data in enumerate(message["content"]["categories"], 1):
                ...
                st.markdown(f"<span style='font-size: 24px;'><strong>Stop {cat_idx}: {category}</strong></span>",
                            unsafe_allow_html=True)
                st.write("")
        ...
        with st.chat_message("assistant"):
            ...
            if not categorized_services:
                ...
                for cat_idx, (category, services) in enumerate(categorized_services.items(), 1):
                    ...
                    if cat_idx == 1:
                        _default_coords = app._city_cfg["default_coords"]
                        ...
                        if is_personal_location:
                            ...
                            if user_location:
                                ...
                                logging.info(
                                    f"First leg (personal location): Using user location for search AND map: {user_location}")
                            else:
                                search_coordinates = _default_coords
                                map_start_coordinates = _default_coords
                                logging.info(
                                    f"First leg (personal location, no permission): Using {app.city} default for search AND map")
                        else:
                            ...
                            if has_specific_location:
                                ...
                                search_coordinates = specific_coords if specific_coords else _default_coords
                                map_start_coordinates = user_location if user_location else _default_coords
                                logging.info(
                                    f"First leg (specific location '{has_specific_location}'): Search from {search_coordinates}, map from {map_start_coordinates}")
                            else:
                                # No location mentioned - use user location for both
                                search_coordinates = user_location if user_location else _default_coords
                    ...
                    # SUBSEQUENT LEGS: Use optimized routing
                        ...
                        if prev_lat and prev_lon:
                            ...
                            last_successful_coordinates = search_coordinates
                            prev_org_name = (
                                        first_result.get('o.name') or first_result.get('name') or 'previous location')
                            logging.info(
                                f"Leg {cat_idx} (optimized): Search AND map from '{prev_org_name}' at {search_coordinates}")
                        else:
                            search_coordinates = last_successful_coordinates
                            map_start_coordinates = last_successful_coordinates
                            logging.warning(
                                f"Leg {cat_idx}: Using last successful coordinates: {last_successful_coordinates}")
                    else:
                        search_coordinates = user_location if user_location else app._city_cfg["default_coords"]
                        map_start_coordinates = search_coordinates
                    ...
                    with st.spinner(f"Searching {category} for {services_text}..."):
                        ...
                        query_result = app.query_service.process_query_with_coordinates(category_query,
                                                                                        search_coordinates)
                    ...
                    if raw_data and len(raw_data) > 0:
                        ...
                        logging.info(
                            f"No results to store. Next iteration will use last successful coordinates: {last_successful_coordinates}")
