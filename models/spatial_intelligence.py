...
class SpatialIntelligence:
    ...
    def __init__(self, city: str = "Philadelphia"):
        ...
        self.landmarks = self._city_cfg["landmarks"]
        # Keep philly_landmarks as alias for backward compatibility
        self.philly_landmarks = self.landmarks
        self.geocode_suffix = self._city_cfg["geocode_suffix"]
        self.zip_pattern = self._city_cfg["zip_pattern"]

        logging.info(f"SpatialIntelligence initialized for city: {city} with {len(self.landmarks)} landmarks")
    ...
    def detect_spatial_query(self, query):
        ...
        # Check for city landmarks
        has_landmarks = any(landmark in query_lower for landmark in self.landmarks.keys())
        if has_landmarks:
            logging.info(f"{self.city} landmark detected in query")
            return True
        ...
        address_patterns = [
            ...
            # Zip codes (city-specific pattern)
            self.zip_pattern
        ]
        ...
        location_patterns = [
            (r'\bin\s+(?!the\s+)(?!a\s+)(?!\d)([a-zA-Z]+(?:\s+[a-zA-Z]+)*)', 'in'),
            # "in Philadelphia" but not "in the morning"
            (r'\bat\s+(?!\d)([a-zA-Z]+(?:\s+[a-zA-Z]+)*)', 'at'),  # "at Temple" but not "at 8pm"
            (r'\bon\s+(?!a\s+)(?!the\s+)(?!\d)([a-zA-Z]+(?:\s+[a-zA-Z]+)*)', 'on'),
            # "on North Broad Street" - FIXED VERSION
        ]
    ...
    def geocode_location(self, location_text):
        ...
        # Check if it's a known city landmark
        ...
        try:
            ...
            search_query = f"{location_text}, {self.geocode_suffix}"
    ...
    def extract_location_from_query(self, query):
        ...
        # --- PRIORITY 1: Check for known city landmarks first ---
        ...
        zip_match = re.search(self.zip_pattern, query_lower)
        if zip_match:
        ...
        for pattern in landmark_patterns:
            ...
            if match:
                ...
                service_words = ['story', 'time', 'toddler', 'program', 'class', 'service', 'form', 'tax',
                                 'application']
        ...
        numbered_address_match = re.search(
            r'\b(\d{1,5}\s+[a-zA-Z]+(?:\s+[a-zA-Z]+){0,2}\s+(?:street|st|avenue|ave|road|rd|blvd|boulevard))\b',
            query_lower)
